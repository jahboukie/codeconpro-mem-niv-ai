...
export class TeamDashboard {
  ...
  constructor(teamMemoryEngine: TeamMemoryEngine, port: number = 3000) {
  ...
  async getMemberDetailedStats(memberId: string): Promise<MemberDashboardStats | null> {
  ...
  private async generateTeamOverview(): Promise<TeamOverview> {
    const members = await this.teamMemoryEngine.getTeamMembers();
    const analytics = await this.teamMemoryEngine.getTeamAnalytics();
    ...
    return {
      teamName: 'Development Team', // This would come from team settings
      memberCount: members.length,
      totalMemories: analytics.totalMemories,
      activeProjects: 5, // This would come from project data
      teamScore,
      trend,
  ...
  private async getActivityFeed(limit: number): Promise<ActivityFeed[]> {
  ...
  private async generateKnowledgeGraph(): Promise<KnowledgeGraphNode[]> {
    ...
    memories.forEach((memory, index) => {
      nodes.push({
        ...
        name: memory.title,
        type: 'memory',
        size: memory.usageCount * 10 + 20, // Size based on usage
        connections: [memory.createdBy], // Connected to creator
        x: Math.random() * 800,
        y: Math.random() * 600,
  ...
  private async getTopMemories(): Promise<TeamMemory[]> {
    const memories = await this.teamMemoryEngine.getTeamMemories();
    return memories
  ...
  private async generateMemberStats(): Promise<MemberDashboardStats[]> {
    const members = await this.teamMemoryEngine.getTeamMembers();
    const analytics = await this.teamMemoryEngine.getTeamAnalytics();
  ...
  // Helper methods
  private calculateOverallTeamScore(analytics: TeamAnalytics): number {
    return Math.round(
      (analytics.teamProductivityScore * 0.3) +
  ...
  private calculateTrend(growthRate: number): 'up' | 'down' | 'stable' {
  ...
  private calculateCollaborationScore(member: TeamMember, analytics: TeamAnalytics): number {
    ...
    return Math.round(
      (memberStats.memoriesCreated * createdWeight) +
      (memberStats.memoriesUsed * usedWeight)
    );
  }

  private calculateRecentActivity(member: TeamMember): number {
    const daysSinceLastActive = (Date.now() - member.lastActive.getTime()) / (1000 * 60 * 60 * 24);
    return Math.max(0, 10 - daysSinceLastActive);
  }

  private calculateStreak(member: TeamMember): number {
    // This would calculate consecutive days of activity
    return Math.floor(Math.random() * 15) + 1; // Placeholder
  }

  private calculateBadges(member: TeamMember, stats: any): string[] {
    const badges: string[] = [];
    
    if (stats?.memoriesCreated > 50) badges.push('Knowledge Creator');
    if (stats?.successScore > 0.8) badges.push('Quality Expert');
    if (stats?.memoriesUsed > 100) badges.push('Active Learner');
    if (member.role === 'admin') badges.push('Team Lead');
    
    return badges;
  }

  private calculateMemberTrend(member: TeamMember): 'up' | 'down' | 'stable' {
